...
const LecturerEvaluations = () => {
  ...
  useEffect(() => {
    const fetchGroups = async () => {
      ...
      try {
        ...
        const res = await axios.get(`/api/groups/modules/${selectedModule}/all`);
        const fetchedGroups = Array.isArray(res.data) ? res.data : [];
        setGroups(fetchedGroups);
        setSelectedGroup('');
      } catch (error) {
        console.error('Failed to fetch groups:', error);
        alert(getErrorMessage(error, 'Failed to load groups for the selected module.'));
        setGroups([]);
        setSelectedGroup('');
      } finally {
  ...
  const submitEvaluation = () => {
    ...
    const moduleData = modules.find((m) => String(m.id) === String(selectedModule));
    const groupData = groups.find((g) => String(g.id) === String(selectedGroup));

    const evaluationData = {
      lecturerId: storedUser?.id,
    ...
    alert(`Evaluation for ${groupData?.groupName || 'selected group'} submitted! Final Score: ${totalScore}/100`);
  };
  ...
  return (
    <div className="max-w-5xl mx-auto p-4">
      ...
      <div className="bg-[#F4F4F6] rounded-4xl shadow-xl border border-gray-200 border-b-8 border-b-[#FF6A00] overflow-hidden">
        <div className="p-8 border-b border-gray-200 bg-white/50">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            ...
            <div>
              ...
              <div className="relative">
                <select
                  className="w-full p-4 pr-12 bg-white text-[#0A0A0C] border border-gray-200 rounded-2xl outline-none focus:border-[#FF6A00] transition-all cursor-pointer font-bold text-sm appearance-none hover:border-[#FF6A00] disabled:bg-gray-100 disabled:cursor-not-allowed"
                  value={selectedGroup}
                  onChange={(e) => setSelectedGroup(e.target.value)}
                  disabled={!selectedModule || loadingGroups}
                >
                  <option value="">
                    {!selectedModule
